          ...
          <div class="btn-group dropup">
            ...
            <div class="dropdown-menu search-list search-box-sink" data-ride="searchList">
                ...
                <div id="selectProject">
                    <span>园区：</span>
                    <select onchange="changeUsers($(this).val())">
                        <?php foreach($teamRows as $row){?>
                        <option <?php echo $projectID == $row->id ? 'selected' : '';?> value="<?php echo $row->id;?>">
                            <?php echo $row->name;?>
                        </option>
                        <?php } ?>
                    </select>
            ...
            <?php endif;?>
              <div class="list-group">
                ...
                foreach($teamMembers as $key => $value)
                {
                    ...
                    $searchKey = $withSearch ? ('data-key="' . zget($membersPinYin, $value->realname, '') . " @$key\"") : "data-key='@$key'";
                    $searchKey .= " root='{$value->root}'";
                    echo html::a("javascript:$(\".table-actions #assignedTo\").val(\"$key\");setFormAction(\"$actionLink\", \"hiddenwin\", \"#taskList\")", $value->realname, '', $searchKey);
                }
                ?>
              </div>
            </div>
...
$(function(){
    ...
    //点击园区切换园区用户
    <?php if($isSuperUser){?>
        changeUsers("<?php echo $projectID;?>");
    <?php } ?>
});

//点击园区切换园区用户
function changeUsers(parkId){
    $('.search-list-item').hide();
    $('.search-list-item[root='+ parkId +']').show();
}
</script>
