...
class projectModel extends model
{
    ...
    public function getTeamMembers($projectID)
    {
        ...
        return $this->dao->select("t1.*, t1.hours * t1.days AS totalHours, if(t2.deleted='0', t2.realname, t1.account) as realname")->from(TABLE_TEAM)->alias('t1')
            ...
            ->fetchAll('account');//printSQL 可以打印sql
    }

    public function getAllTeamMembers($projectID)
    {
        return $this->dao->select("t1.*, t1.hours * t1.days AS totalHours, if(t2.deleted='0', t2.realname, t1.account) as realname")->from(TABLE_TEAM)->alias('t1')
            ->leftJoin(TABLE_USER)->alias('t2')->on('t1.account = t2.account')
    ...
    public function getTeams($projectID=0)
    {
        $userGroups = $this->app->user->groups;
        $superUsers = [1,25,16];//园区负责人
        $isLeader = empty(array_intersect($superUsers,$userGroups)) ? false : true;

        $this->dao->select("t1.id,t1.team name")->from(TABLE_PROJECT)->alias('t1')
            ->where('t1.deleted')->eq(0);
        //如果是园区负责人则只显示本园区和总裁办
        if ($isLeader) {
            $this->dao->andWhere('t1.id')->in("{$projectID},4");
        }    
        // $datas->printSQL();
        $datas = $this->dao->fetchAll();
        // ee($datas);
        return $datas;
    }
    ...
    public function getTeamMemberPairs($projectID, $params = '')
    {
        ...
        // ee($users);
        //过滤总裁办的人员
        $this->loadModel('user')->removeCeoTeamMembers($users,1,$projectID);
