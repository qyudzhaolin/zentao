...
class project extends control
{
    ...
    public function task($projectID = 0, $status = 'unclosed', $param = 0, $orderBy = '', $recTotal = 0, $recPerPage = 100, $pageID = 1)
    {
        ...
        //二开添加
         ...
         //获取权限
        $isSuperUser = $this->loadModel('user')->isSuperUser();
       ...
       // ee( $tasks);
        if(!$isSuperUser){
            ...
            foreach ($tasks as $key => $value) {
               if($value->openedBy==$account||$value->assignedTo==$account){
                  $newstasks[]=$tasks[$key];
               }
               if (!empty($value->children)) {
                   foreach ($value->children as $k => $v) {
                       if($v->openedBy== $account||$v->assignedTo== $account){
                          $newstasks[]=$value->children[$k];
                       }
                   }
               }
       ...
       /* Build the search form. */
        ...
        //是否为超级用户(如果是管理员或者园区负责人则为超级用户)
        if ($isSuperUser) {
            $this->view->teamRows    = $this->project->getTeams($projectID);
            $this->view->teamMembers = $this->project->getAllTeamMembers();
        }
        //过滤总裁办的人员
        $this->loadModel('user')->removeCeoTeamMembers($this->view->teamMembers);


        // ee($this->view->teamMembers);
        ...
        if ($isSuperUser) {
            $memberPairs = [];
            foreach($this->view->teamMembers as $key => $member) {
                $memberPairs[$member->account] = $member->realname;
            }
        }
        // ee($memberPairs);
        ...
        /* Assign. */
        $this->view->isSuperUser         = $isSuperUser;
