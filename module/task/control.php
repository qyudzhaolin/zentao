...
class task extends control
{
    ...
    public function create($projectID = 0, $storyID = 0, $moduleID = 0, $taskID = 0, $todoID = 0)
    {
        ...
        //过滤总裁办的人员
        $this->loadModel('user')->removeCeoTeamMembers($members,1);
        // ee($members);
    ...
    public function edit($taskID, $comment = false)
    {
        ...
        if ($isSuperUser) {
            $this->view->teamRows = $this->project->getTeams($this->view->task->project);
        }


        if(!isset($this->view->members[$this->view->task->assignedTo])) $this->view->members[$this->view->task->assignedTo] = '';
