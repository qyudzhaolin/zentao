...
class userModel extends model
{
    ...
    public function isSuperUser()
    {
        ...
        $superUsers = [1,16,25];
        $isSuperUser = empty(array_intersect($superUsers,$userGroups)) ? false : true;
        return $isSuperUser;
    }

    //获取总裁办员工
    public function getCeoTeamMembers()
    {
        $rows = $this->dao->select("t1.account")->from(TABLE_USERGROUP)->alias('t1')
            ->leftJoin(TABLE_USER)->alias('t2')->on('t1.account = t2.account')
            ->where('`t1`.`group`')->eq(16)
            ->fetchAll('account');
        return $rows;
    }

    //从员工组中去掉总裁办员工
    public function removeCeoTeamMembers(&$rows, $isKV=0,$projectID=0)
    {
        if (empty($rows)) {
            return null;
        }
        $ceoRows = $this->getCeoTeamMembers();
        $ceoRows = array_keys($ceoRows);
        // ee($rows);
        array_push($ceoRows, 'admin');
